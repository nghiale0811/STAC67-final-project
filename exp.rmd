...
decisiontree <- rpart(`Rented Bike Count`~.-Date-`Dew point temperature(°C)`-`Solar Radiation (MJ/m2)`-`Snowfall (cm)` - `Functioning Day`, data = n_data_train, method = 'anova')
pred = predict(decisiontree, n_data_test, method = "anova")

#RMSE
RMSE(pred = pred, obs = n_data_test$`Rented Bike Count`)
#R^2
cor(n_data_test$`Rented Bike Count`, pred) ^ 2
